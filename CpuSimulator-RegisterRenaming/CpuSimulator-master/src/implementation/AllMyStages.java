...
//                globals.setClockedProperty("branch_state_decode", GlobalData.BRANCH_STATE_NULL);
           ...
           /* if (opcode.needsWriteback()) {
            ...
            GlobalData globals1 = (GlobalData)getCore().getGlobals();
           // GlobalData gdataObj = new GlobalData();
            if(src1.isRegister())
            {
            	int arc_regName = src1.getRegisterNumber();
            	int physregnum = globals1.rat[arc_regName];
            	src1.rename(physregnum);
            }
            if(src2.isRegister())
            {
            	int arc_regName = src2.getRegisterNumber();
            	int physregnum = globals1.rat[arc_regName];
            	src2.rename(physregnum);
            }
           System.out.println( opcode.toString());
           if(!opcode.toString().equals("MOV")&&!opcode.toString().equals("CALL"))
           {
            if (opcode.oper0IsSource() && oper0.isRegister())
            {
            	int arc_regName = oper0.getRegisterNumber();
            	int physregnum = globals1.rat[arc_regName];
            	oper0.rename(physregnum);
            }
           }
...
//                            Logger.out.println("Stall JMP wants oper0 R" + oper0.getRegisterNumber());
                    ...
                    if (opcode.needsWriteback()&& (!opcode.oper0IsSource()))
                    {
                    	int archRegName = oper0.getRegisterNumber();
                        //regfile.markRenamed(globals1.rat[ins.getOper0().getRegisterNumber()], true);
                        regfile.markRenamed(globals1.rat[archRegName], true);
                        int phy_reg =-1;
                        for(int i=0;i<256;i++)
                        {
                        	if(!regfile.isUsed(i))
                        ...
                        regfile.changeFlags(phy_reg,  1|4, 2|8);
                        globals1.rat[ins.getOper0().getRegisterNumber()] =  phy_reg;
                        oper0.rename(phy_reg);
                    }
                    globals.setClockedProperty("program_counter_takenbranch", value1);
                    globals.setClockedProperty("branch_state_decode", GlobalData.BRANCH_STATE_TAKEN);
                    globals.setClockedProperty("decode_squash", true);
...
//                    return;
            ...
            if (opcode.needsWriteback()&& (!opcode.oper0IsSource())) {
                ...
                int oper0reg = oper0.getRegisterNumber();
              //  regfile.markRenamed(globals1.rat[ins.getOper0().getRegisterNumber()], true);
                regfile.markRenamed(globals1.rat[oper0reg], true);
                int phy_reg =-1;
                for(int i=0;i<256;i++)
                {
                	if(!regfile.isUsed(i))
    ...
    static class Writeback extends PipelineStageBase {
        ...
        public void compute() {
            ...
            for (int i=0; i<num_inputs; i++) {
                ...
                for (int j = 0; j < 256; j++) {
                    if (!regfile.isInvalid(j) && regfile.isRenamed(j)) {
                        regfile.markUsed(j, false);
                    }
                }
